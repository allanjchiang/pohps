# POHPS documentation

| File | Purpose |
|------|---------|
| [support.html](./support.html) | Support, privacy policy, and disclaimer for store listings |
| [google-play-data-safety.md](./google-play-data-safety.md) | Line-by-line Google Play Data safety form answers |

## Publishing URLs

- **Support URL:** https://logicphile.com/pohps/
- **Privacy policy:** https://logicphile.com/pohps/
- **Support email:** allan@logicphile.com

Deploy `support.html` to your web host so it is served at the support URL above (e.g. copy to your site’s `/pohps/` directory as `index.html`).
